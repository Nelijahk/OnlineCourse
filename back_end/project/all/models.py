...
class User(baseModel):
    ...
    status = Column(Enum('Teacher', 'Student', 'Admin'), nullable=False)
    activate = Column(Enum('Yes', 'No'), nullable=False)
    ...
    def __init__(self, **kwargs):
        ...
        self.activate = kwargs.get('activate')
    ...
    @classmethod
    def authentication(cls, user_name, password):
        user = cls.query.filter(cls.user_name == user_name).one()
...
class Course(baseModel):
    ...
    id_teacher = Column(VARCHAR(128), ForeignKey('user.user_name'), nullable=False)
...
class User_and_Course(baseModel):
    ...
    user_id = Column(VARCHAR(128), ForeignKey('user.user_name'), nullable=False)
    id_courses = Column(VARCHAR(128), ForeignKey('course.name'), nullable=False)
